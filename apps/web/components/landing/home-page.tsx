...
import { hasReturningCookie } from "@/lib/dc-cookie";
...
export function HomePage({
  ...
  useEffect(() => {
    createClient().auth.getSession().then(({ data: { session } }) => {
      ...
      setShowLogin(isLoggedIn || (isLoggedIn === false && hasReturningCookie()));
      setCtaHref(isLoggedIn ? "/dashboard" : "/login");
    });
  }, []);
    ...
    <div className="min-h-screen bg-[#fafaf9] relative overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 w-full border-b border-black/5 bg-[#fafaf9]/95 backdrop-blur-md">
        <div className="mx-auto flex h-14 sm:h-16 max-w-5xl w-full items-center justify-between px-4 sm:px-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4 text-sm">
            ...
            <Link
              href={showLogin ? ctaHref : "/sign-up"}
            ...
            >
              {showLogin ? ctaLabel : "Get started"}
            </Link>
            <button
              type="button"
        ...
        <div className="relative w-full">
          ...
          >
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
              ...
              >
                {showLogin ? ctaLabel : "Get started free"}
              </Link>
            </motion.div>
            <a
              href="#how-it-works"
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          <div className="mt-16 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4">
              {[
                { icon: Video, iconBg: "#e8f0fe", iconColor: "#2D8CFF", trigger: "Call ends", action: "Insight extracted", example: "Product sync → 3 key takeaways", stat: "~2 min" },
                { icon: MessageSquare, iconBg: "#f5e6f5", iconColor: "#4A154B", trigger: "Thread deepens", action: "Pattern detected", example: "5 messages → consolidated view", stat: "Real-time" },
                { icon: FileText, iconBg: "#e8f0fe", iconColor: "#4285F4", trigger: "Doc updated", action: "Change captured", example: "New metrics → draft prompt", stat: "Instant" },
                { icon: Calendar, iconBg: "#ccfbf1", iconColor: "#0d9488", trigger: "Weekly wrap", action: "Digest ready", example: "7 insights → pick & draft", stat: "Friday" },
              ].map((item, i) => {
                const Icon = item.icon;
                return (
                  <motion.div key={item.trigger} {...stagger} transition={{ delay: i * 0.1 }} className="relative flex flex-col items-start text-left">
                    <motion.div whileHover={{ scale: 1.02, y: -2 }} className="relative z-10 w-full rounded-xl border border-zinc-200 bg-white p-6 shadow-sm hover:shadow-lg hover:border-[var(--accent)]/30 transition-all">
                      <div className="flex h-14 w-14 items-center justify-center rounded-xl" style={{ backgroundColor: item.iconBg }}>
                      ...
                      <span className="mt-3 inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600">{item.stat}</span>
                    </motion.div>
                    {i < 3 && (
          ...
          <div className="mt-12 grid md:grid-cols-2 gap-8 md:items-stretch">
            <motion.div {...stagger} transition={{ delay: 0.15 }} className="flex flex-col space-y-4">
              ...
              {[
                ...
                const Icon = item.icon;
                return (
                  <motion.div key={i} {...stagger} transition={{ delay: 0.2 + i * 0.05 }} whileHover={{ scale: 1.02, y: -2 }} className="flex gap-4 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm hover:shadow-lg hover:border-[var(--accent)]/20 transition-all">
            ...
            <motion.div {...stagger} transition={{ delay: 0.2 }} className="flex flex-col space-y-4">
              ...
              {[
                ...
                const Icon = item.icon;
                return (
                  <motion.div key={i} {...stagger} transition={{ delay: 0.25 + i * 0.05 }} whileHover={{ scale: 1.02, y: -2 }} className="flex gap-4 rounded-xl border-l-4 bg-white p-5 shadow-sm hover:shadow-lg transition-all" style={{ borderLeftColor: item.color }}>
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: item.bg }}>
                      <Icon className="h-6 w-6" style={{ color: item.color }} />
                    </div>
                    <div>
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
            {[
              ...
              const Icon = f.icon;
              return (
                <motion.div key={f.title} {...stagger} transition={{ delay: i * 0.06 }} whileHover={{ scale: 1.02, y: -2 }} className="rounded-xl border border-zinc-200 bg-white p-6 hover:shadow-lg hover:border-[var(--accent)]/20 transition-all">
      ...
      <section className="relative border-t border-zinc-200 px-4 sm:px-6 py-12 sm:py-16 md:py-24 overflow-hidden z-[1] bg-[#fafaf9]">
        <div className="relative mx-auto max-w-5xl">
          ...
          <motion.div {...stagger} transition={{ delay: 0.1 }} className="mt-12 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl shadow-zinc-200/50 ring-1 ring-zinc-200/50">
      ...
      <section id="pricing" className="relative border-t border-zinc-200 px-4 sm:px-6 py-12 sm:py-16 md:py-24 overflow-hidden z-[1] scroll-mt-20 bg-white">
        <div className="relative mx-auto max-w-5xl">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {[
              { name: "Free", price: "$0", period: "", desc: "Try it out", features: ["1 Slack workspace", "2 sources (Slack + Zoom or Google)", "10 insights/month", "Basic tone", "Dashboard"], cta: "Get started free", href: "/sign-up", highlighted: false },
              { name: "Pro", price: annual ? "$23" : "$29", period: "/mo", desc: "For individuals", features: ["Unlimited insights", "All sources (Slack, Zoom, Google, Gmail, repos)", "Tone filters: Educational, Inbound, Tactical", "Publish to LinkedIn", "Voice match", "Sensitivity controls"], cta: "Start Pro trial", href: "/sign-up", highlighted: true },
              { name: "Business", price: "Custom", period: "", desc: "For teams", features: ["Everything in Pro", "Team workspace", "Role-based access", "Code repos & dev tools", "Priority support", "Custom integrations"], cta: "Contact sales", href: "https://calendly.com/1sankalp", highlighted: false },
            ].map((plan, i) => (
              <motion.div
                key={plan.name}
              ...
              >
                ...
                {plan.href.startsWith("http") ? (
                  <a href={plan.href} target="_blank" rel="noopener noreferrer" className={`mt-6 block w-full rounded-lg py-3 text-center font-semibold transition-all duration-200 ${plan.highlighted ? "bg-[var(--accent)] text-white hover:opacity-90 hover:scale-[1.02]" : "border border-zinc-300 text-zinc-900 hover:bg-zinc-50"}`}>
                    {plan.cta}
                  </a>
                ) : (
                  <Link href={plan.href} className={`mt-6 block w-full rounded-lg py-3 text-center font-semibold transition-all duration-200 ${plan.highlighted ? "bg-[var(--accent)] text-white hover:opacity-90 hover:scale-[1.02]" : "border border-zinc-300 text-zinc-900 hover:bg-zinc-50"}`}>
                    {plan.cta}
                  </Link>
                )}
              </motion.div>
      ...
      {/* For Enterprise */}
      <section className="relative border-t border-zinc-200 px-4 sm:px-6 py-12 sm:py-16 md:py-24 overflow-hidden z-[1]">
        <div className="relative mx-auto max-w-5xl">
          <motion.span {...fadeUp} className="text-xs font-semibold text-[var(--accent)] uppercase tracking-wider">
            For Enterprise
          </motion.span>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <motion.div key={stat.label} {...stagger} transition={{ delay: 0.1 + i * 0.05 }} className="rounded-xl border border-zinc-200 bg-white p-6 hover:border-[var(--accent)]/30 transition-colors">
          ...
          <div className="mt-10">
            <a href="https://calendly.com/1sankalp" target="_blank" rel="noopener noreferrer" className="inline-block rounded-lg bg-[var(--accent)] px-6 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity">
      ...
      <footer className="relative w-full border-t border-zinc-200 py-8 z-[1] bg-white">
        <div className="mx-auto flex max-w-5xl w-full flex-col sm:flex-row items-start sm:items-center justify-between gap-6 px-4 sm:px-0">
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 font-display text-sm text-zinc-600 leading-relaxed">
            ...
            <a href="https://calendly.com/1sankalp" target="_blank" rel="noopener noreferrer" className="hover:text-[var(--accent)] transition-colors pb-px">Contact</a>
          </div>
        </div>
      </footer>
    </div>
